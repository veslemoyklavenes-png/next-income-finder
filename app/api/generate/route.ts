...
Section 4, Your First Move: Start with one single bold sentence telling them exactly what to do first — the most important action, not a list, one move. Then on a new line, write a ready-to-use outreach message they can send this week to start Option 1. Label it clearly as "Message you can send this week:" and write it as a complete, natural-sounding LinkedIn message or email (whichever fits better) addressed to a real person in their network. The message should be 3-5 sentences, written in first person, warm and direct, not salesy. It should mention what they are offering, why they are reaching out to that specific type of person, and end with a soft question. Never use placeholders like [Name] — write it so the user only needs to swap in the actual name.
